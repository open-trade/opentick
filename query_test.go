...
func Test_Query(t *testing.T) {
	...
	assert.Equal(t, []interface{}{int64(2), int64(1), true, int64(42), 2.2, int64(102)}, res[0])
	res, err1 = Execute(db, "", "alter table test.test rename to test2", nil)
	res, err1 = Execute(db, "", "select * from test.test where a=2 and b=1 and b2=true and c>39 and c<42", nil)
	assert.Equal(t, "Table test.test does not exists", err1.Error())
	res, err1 = Execute(db, "", "select * from test.test2 where a=2 and b=1 and b2=true and c>39 and c<42", nil)
	assert.Equal(t, nil, err1)
	assert.Equal(t, 1, len(res))
	assert.Equal(t, []interface{}{int64(2), int64(1), true, int64(41), 2.2, int64(104)}, res[0])
	res, err1 = Execute(db, "", "alter table test.test2 rename to test", nil)
	res, err1 = Execute(db, "", "select * from test.test where a=2 and b=1 and b2=true and c>=39 and c<=42", nil)
	assert.Equal(t, nil, err1)
	assert.Equal(t, 3, len(res))
	...
	LoadUsers(db)
	_, err1 = Execute(db, "", "insert into _meta_.user values('__a', 'x', true, 'a=write;b.x=read')", nil)
	_, err1 = Execute(db, "", "insert into _meta_.user values('__b', 'y', false, 'a=read;b.x=write')", nil)
	assert.Equal(t, nil, err1)
	LoadUsers(db)
	u, _ := userMap.Load("__a")
	assert.Equal(t, true, u.(*User).isAdmin)
	assert.Equal(t, WritablePerm, int(u.(*User).perm["a"]))
	u, _ = userMap.Load("__b")
	assert.Equal(t, false, u.(*User).isAdmin)
	assert.Equal(t, ReadablePerm, int(u.(*User).perm["a"]))
	_, err1 = Execute(db, "", "delete from _meta_.user where name='__a'", nil)
	_, err1 = Execute(db, "", "delete from _meta_.user where name='__b'", nil)
